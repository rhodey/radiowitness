...
const wssServer   = require('./lib/wss-server.js')
const wssMirror   = require('./lib/wss-mirror.js')
...
function dbPathFor(call) {
  const hour = Math.floor(call.startms / 1000.0 / 60 / 60)
...
const defaults = {
  ...
  dat: { valueEncoding: 'utf-8' },
  dirAudio: '/archive/pcm16bit8kHz', dirDb: '/archive/db', partition: 10000000, limit: 0,
  wss: 8081
}
...
switch (cmd0) {
  ...
  case 'play':
    ...
    pull(
      ...
      pull.drain(function (call) {
        ...
        const lengthms = Math.floor((wav.length * 8) / 16)
  ...
  case 'create':
    ...
    mkdirp(opts.dirAudio)
      ...
      .then(function () {
        audio = hypercore(storageAudio(opts), opts.dat)
        db = hyperdb(opts.dirDb, opts.dat)
  ...
  case 'archive':
    ...
    Promise.all([ready(audio), ready(db)]).then(function () {
      ...
      wssServer(opts.wss, audio, db, opts.limit)
      pull(
        toPull.source(process.stdin.pipe(split())),
        pull.map(codec.decode),
        p25buffer(opts.callms),
        ...
        pull.drain(function (call) {
          ...
          append = append.then(() => limitAppend(audio, opts.limit, wav)).then(function (seq) {
            ...
            delete call.wav
            db.put(dbPathFor(call), call, closeIfErr)
          }).catch(onClose)
        }, onClose)
      )
    })
  ...
  case 'mirror':
    process.stdin.once('end', onClose)
    wssMirror.fetchConfig(cmd1).then(function (remote) {
      audio = hypercore(storageAudio(opts), remote.audio, Object.assign(opts.dat, { sparse: true }))
      db = hyperdb(opts.dirDb, remote.db, Object.assign(opts.dat, { sparse: false }))
      return Promise.all([ready(audio), ready(db)]).then(function () {
        wssServer(opts.wss, audio, db, opts.limit)
        return wssMirror.replicate(cmd1, audio, db, opts.limit, remote.limit)
      })
    }).catch(onClose)
    break;
