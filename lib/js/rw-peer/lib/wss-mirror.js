...
function replicate(host, audio, db, limitLocal, limitRemote) {
  // todo: reconnect
  return new Promise(function (res, rej) {
    const audioWss = websocket(`ws://${host}/audio`)
    audioWss.on('error', rej)
    const audioRpl = audio.replicate(true, { live: true })
    audioRpl.pipe(audioWss).pipe(audioRpl)
    ...
    const dbWss = websocket(`ws://${host}/db`)
    dbWss.on('error', rej)
    const dbRpl = db.replicate({ live: true })
    dbRpl.pipe(dbWss).pipe(dbRpl)
  })
}
